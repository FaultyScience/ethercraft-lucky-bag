...
contract AltRNGCore {
  ...
  function setBlockhash() private {
    ...
    newLength = queuedBlocks.length - processedBlockCount;

    for (i = 0; i < newLength; i++) {
      queuedBlocks[i] = queuedBlocks[i + processedBlockCount];
    }
  ...
  function getBool(uint256 _block) public view returns (bool) {
    return uint256(getHash(_block)) % 2 == 0;
  }
